...
Mica1's hardware is built around three primary components:

**ESP32-S3 Microcontroller (~$3–5)**
The ESP32-S3 is the brain. It runs our on-device inference engine (TensorFlow Lite for Microcontrollers), manages sensor fusion, handles BLE communication, and controls power states. It's a remarkable piece of silicon for the price — dual-core Xtensa LX7 at 240MHz, with a vector extension that accelerates neural network inference by 40× over the base ESP32.

**IMU + Bio-Sensor Array (~$4–7)**
For motion and biomechanical sensing (orthopedic, activity, and fall-detection modes), we use a 6-axis IMU — either the LSM6DSO or ICM-42688-P depending on availability. For cardiovascular and fluid-shift modes, we combine a MAX30102 PPG module with custom bioelectrical impedance analysis (BIA) electrodes.

**Power Management + Battery (~$3–5)**
A 3.7V 300mAh LiPo cell with a TP4056-based charging circuit. Our deep-sleep power management firmware achieves sub-50µA quiescent draw, enabling the 3–5 day battery life target.

The remaining budget covers PCB fabrication, passive components, antenna matching, and the enclosure.
...
*Mica1 is a product of Nezaira, built in Xi'an, China. We're building intelligent health systems for Earth and beyond.*
